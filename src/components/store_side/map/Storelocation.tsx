...
import { Store_put_Location } from "../../../reducers/autopartsstore/autopartsStoreapicalls";
...
const StoreCurrentLocation = () => {
  ...
  const handleConfirm = () => {
    if (selectedLocation && address && id) {
      ...
      dispatch(Store_put_Location({ id: id, location: data }))
        .unwrap()
        .then(() => {
          navigate(-1);
        });
...
export default StoreCurrentLocation;
