...
import { Cart, ErrorPayload } from '../../../types/clients/UsersTypes';
import { User_get_CartnyId, User_Get_storeDetails } from '../../../reducers/users/UserapiCalls';
...
const Checkout: React.FC = () => {
  ...
  useEffect(() => {
    if (cartId) {
      dispatch(User_get_CartnyId(cartId))
        .unwrap()
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-white p-8 rounded-lg shadow-lg">
            ...
            <form onSubmit={handleSubmit}>
                ...
                {['razorpay', 'stripe', 'wallet'].map((method) => (
                  <label key={method} className="flex items-center space-x-3">
                    <input
                      ...
                      onChange={() => handlePaymentMethodChange(method as 'razorpay' | 'stripe' | 'wallet')}
              ...
              <button type="submit" className="w-full mt-6 bg-indigo-600 text-white py-3 rounded-md hover:bg-indigo-700 transition">
                Place Order
              </button>
            </form>
          </div>
          <div className="bg-white p-8 rounded-lg shadow-lg">
            ...
            {isLoaded && showMap && storeDetails?.location && user?.location ? (
              <GoogleMap
                ...
                center={{ lat: (storeDetails.location.lat + user.location.lat) / 2, lng: (storeDetails.location.lng + user.location.lng) / 2 }}
                zoom={12}
              >
