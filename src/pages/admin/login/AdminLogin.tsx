...
import { adminLoginPost } from "../../../reducers/admin/adminapicalls";
import { reset } from "../../../reducers/admin/adminReducers";
import { useNavigate } from "react-router-dom";
import { toast } from "react-toastify";
import { AppDispatch, RootState } from "../../../store/store";
import { useDispatch, useSelector } from "react-redux";
import { AdminLoginTypes } from "../../../types/admin/admintypes";

const AdminLogin = () => {
    const { isError, isSuccess, message } = useSelector((state: RootState) => state.admin);
    const [logindata,setLogindata]=useState<AdminLoginTypes>({email:"",password:""})
  ...
  if (handleValidate()) {
    ...
    dispatch(adminLoginPost(logindata))
  }
    }
  return (
    <>
 <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-100 to-blue-300 font-sans">
  ...
  <div className="flex flex-col lg:flex-row bg-white rounded-xl shadow-lg overflow-hidden max-w-4xl">
    ...
    >
      <div className="p-8 text-center text-white bg-opacity-50 rounded-lg backdrop-filter backdrop-blur-0">
    ...
    <div className="flex flex-1 flex-col justify-center p-8 lg:p-16">
      <h2 className="text-3xl font-semibold text-blue-600 text-center mb-8">Log In</h2>
      <form className="flex flex-col gap-6">
        ...
        <span className="text-red-600 text-sm">{loginError.emailerr}</span>
        ...
        <button
          className="w-full py-3 bg-blue-500 text-white font-bold rounded-lg hover:bg-blue-600 transition duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
